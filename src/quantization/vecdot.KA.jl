...
function matmul!(
    ...
    KernelAbstractions.synchronize(KernelAbstractions.get_backend(y))
    return nothing
end

@kernel function compute_sums_kernel!(x, sums, num_blocks, sum_size)
...
function _vecdot_hack(scale, sums::AbstractVector, i, d_all)
...
function vecdot_q6_ka(A, idx, x, x_sums)
    nb = size(A, 1)
...
function vecdot_ka!(y::AbstractVector{Float32}, A::AbstractMatrix{block_q4_K}, x, x_sums::AbstractVector{Float16})
    ...
    kernel! = vecdot_q4_kernel!(KernelAbstractions.get_backend(y))
    kernel!(y, A, x, x_sums, ndrange=N)
...
@kernel function vecdot_q4_kernel!(y, @Const(A), @Const(x), @Const(x_sums))
    ...
    if idx <= length(y)
        y[idx] = vecdot_q4_ka(A, idx, x, x_sums)
    end
end

function vecdot_q4_ka(A, idx, x, x_sums)
...
function vecdot_q5_ka(A, idx, x, x_sums)
    nb = size(A, 1)
...
function vecdot_nonvectorized(x::AbstractArray{block_q6_K}, idx, orig::AbstractVector{Float16}, sums::AbstractVector{Float16})
    # The body of this function remains largely unchanged
    # Only change CuVector to AbstractVector
end
